<template>
        <el-dialog v-if="loginDialogVisible" model-value title="登录" width="20%" @close="close" draggable close-delay=150 open-delay=10>
            <div v-if="!isQRlogin">
                <!-- <div class="dialog">
                    <span>账号: </span>
                    <el-input class="input" v-model="user.email" placeholder=" 请输入邮箱" >
                        <template #append>@163.com</template>
                    </el-input>
                </div>
                <div class="dialog">
                    <span>密码: </span>
                    <el-input class="input" type="password" v-model="user.password" show-password placeholder="请输入密码" />
                </div> -->
                <div class="dialog">
                    <span>手机号: </span>
                ...
                <div class="dialog">
                    ...
                    <el-input class="input" v-model="user.captcha" placeholder="请输入验证码">
                        <template #append>
                            <el-button class="send-out" :disabled='isSend' @click="verification(user.phone)">{{ send_out }}</el-button>
                        </template>
                    </el-input>
                </div>
            </div>
...
import { useStore } from 'vuex';
    ...
    const status = async(cookie)=>{
        ...
        console.log(loginStatus.data.data.profile)
    }
